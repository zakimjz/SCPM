...
#	runtimeEpsilon.sh: Compares the naive algorithm, and scpm (using BFS and DFS) 
#	in terms of runtime varying the minimum epsilon parameter.
#	@Arlei Silva

#Including the parameter settings
source runtime_parameters.sh

#Values of minimum epsilon
min_epsilon_values=(0 0.05 0.10 0.15 0.20)

rm runtime_epsilon.dat
...
do
  ...
  echo "$min_epsilon	$avg_time_scpm_bfs	$avg_time_scpm_dfs	$avg_time_naive" >> runtime_epsilon.dat
done
