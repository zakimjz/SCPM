...
#	runtimeSigma.sh: Compares the naive algorithm, and scpm (using BFS and DFS) 
#	in terms of runtime varying the minimum sigma (support) parameter.
#	@Arlei Silva

#Including the parameter settings
source runtime_parameters.sh

#Values of minimum sigma
min_sigma_values=(100 150 200 250 300 350)

rm runtime_sigma.dat

for min_sigma in ${min_sigma_values[@]}
do
  ...
  echo "$min_sigma	$avg_time_scpm_bfs	$avg_time_scpm_dfs	$avg_time_naive" >> runtime_sigma.dat
done
